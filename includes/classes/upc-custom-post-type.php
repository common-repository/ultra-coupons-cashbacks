...
/**
 *UPC Custom Post type
 * This class is used to register a new post type.
 *
 * @since 1.0
 * @author Jayanthi
 */
class UPC_Custom_Post_Type {
	...
	public function __construct( $custom_post_type ) {
		...
		add_filter( 'enter_title_here', array( $this, 'upc_change_enter_title' ) );
	 ...
	 * @since 1.0
	 */
	public static function post_type_labels( $name_singular, $name_plural, $t_domain,$menu_name ) {
		...
		$labels = array(
			...
			'menu_name'          => sprintf( esc_html__( '%s', $t_domain), $menu_name ),
	...
	public static function post_type_register( $post_type, $name_singular, $labels, $t_domain ) {
		...
		$args = array(
			...
			'menu_position'      => 55,
			'menu_icon'          => 
 UPC_Plugin::instance()->plugin_assets . 'img/coupon24.png',
			'has_archive'        => true,
			'show_in_menu'       => true,
	 ...
	 * @since 1.0
	 */
	function upc_change_enter_title( $input ) {

		if ( is_admin() && 'upc_coupons' === get_post_type() ) {
			return __( 'Enter Coupon title here', 'upc-coupon' );
		}
